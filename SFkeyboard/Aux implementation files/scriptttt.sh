...
do
	...
	do
		...
		for iter in {1..100} #se for heuristica vc vai querer rodar mais de uma vez
		do
			echo "Solving: " ${arquivo} ":" #output pra saber que instancia ta sendo rodada
			./exeSFkey ${dirInstancias}/${arquivo} charlists/Char${idioma}.txt KeybLayout/b_matrix.txt
		done
	done
done
