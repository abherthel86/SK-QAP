//Esse main é usado para otimizar caracteres com o layout físico do teclado FITALY
...
/*	qwertyAssign (&data, &keyb);
	...
	bestSpVec(&data, &keyb, bestSpaces, spaceKeys, charKeys);
//******************ini
						// ****cout << "BKS: " << bks << endl;
						// getchar();
						...
						while (mscount < msMax){
							...
							//initSol (&data, &keyb);
							initSol2 (&data, &keyb, spaceKeys, charKeys, bestSpaces);
							...
							while (iterILS <= maxIterILS){
								...
								mtSF_RVND (&data, &keyb, &stat, bestSpaces, spaceKeys, charKeys);

								// long long testCost;
								// testCost = multispaceCalc(&data, &keyb);
								// cout << "Custo calculado final (RVND): " << testCost << endl;
								
								// printSol(data, keyb, auxKeyb);
								// cout << endl;
								//getchar();
								//mswap (&data, &keyb, &stat, bestSpaces, spaceKeys, charKeys);
								//mswapPair (&data, &keyb, &stat, bestSpaces, spaceKeys, charKeys);
								...
								if (keyb.cost < bestSolCost) {
							 	...
							 	mperturbation(&data, &keyb, &pertChoice, bestSpaces, spaceKeys, charKeys);
							 	iterILS++;
							}
...
//****************ini
						...
						cout << "\n-----x-----" << "\nBest Solution: " << endl;
						printSol(&data, &keyb, bestGlobalSol);
						//printSolHex(&data, &keyb, bestGlobalSol);
