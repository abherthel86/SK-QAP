		...
		for (int j = i + 1; j < vecpairs.size(); j++){
			if (vecpairs[i].vert > 0 && vecpairs[j].vert > 0){
				if (vecpairs[j].pos != vecpairs[i].vpair){
					...
					rmvp1 = rmvCost[keyb->grid[vecpairs[i].vpair]];
					rmvp2 = rmvCost[keyb->grid[vecpairs[j].vpair]];
					...
					constVal = ((data->freq_matrix[keyb->grid[vecpairs[i].pos]][keyb->grid[vecpairs[i].vpair]] - 
								data->freq_matrix[keyb->grid[vecpairs[i].pos]][keyb->grid[vecpairs[j].vpair]] + 
								data->freq_matrix[keyb->grid[vecpairs[j].pos]][keyb->grid[vecpairs[j].vpair]] - 
								data->freq_matrix[keyb->grid[vecpairs[j].pos]][keyb->grid[vecpairs[i].vpair]])*
								(data->b_matrix[keyb->assignment[keyb->grid[vecpairs[j].pos]]][keyb->assignment[keyb->grid[vecpairs[i].vpair]]] - 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[j].pos]]][keyb->assignment[keyb->grid[vecpairs[j].vpair]]] + 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[i].pos]]][keyb->assignment[keyb->grid[vecpairs[j].vpair]]] - 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[i].pos]]][keyb->assignment[keyb->grid[vecpairs[i].vpair]]]));

					deltaFi = - rmvp1 - rmvp2 + insp1 + insp2;

					s_Cost = f_Cost + deltaFi + constVal;
			...
			if (vecpairs[i].horiz > 0 && vecpairs[j].horiz > 0){
				if (vecpairs[j].pos != vecpairs[i].hpair){
					...
					rmv2 = rmvCost[keyb->grid[vecpairs[j].pos]];
					rmvp1 = rmvCost[keyb->grid[vecpairs[i].hpair]];
					rmvp2 = rmvCost[keyb->grid[vecpairs[j].hpair]];
					...
					constVal = ((data->freq_matrix[keyb->grid[vecpairs[i].pos]][keyb->grid[vecpairs[i].hpair]] - 
								data->freq_matrix[keyb->grid[vecpairs[i].pos]][keyb->grid[vecpairs[j].hpair]] + 
								data->freq_matrix[keyb->grid[vecpairs[j].pos]][keyb->grid[vecpairs[j].hpair]] - 
								data->freq_matrix[keyb->grid[vecpairs[j].pos]][keyb->grid[vecpairs[i].hpair]])*
								(data->b_matrix[keyb->assignment[keyb->grid[vecpairs[j].pos]]][keyb->assignment[keyb->grid[vecpairs[i].hpair]]] - 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[j].pos]]][keyb->assignment[keyb->grid[vecpairs[j].hpair]]] + 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[i].pos]]][keyb->assignment[keyb->grid[vecpairs[j].hpair]]] - 
								data->b_matrix[keyb->assignment[keyb->grid[vecpairs[i].pos]]][keyb->assignment[keyb->grid[vecpairs[i].hpair]]]));
...
//Bloco de funcoes para teclados com multiplos espacos (sem avaliacao eficiente de movimentos)
...
void mswap (Gendata *data, Keyboard *keyb, Stats *stat, vector<int>& bSpaces, vector<int>& spaceVec, vector<int>& charVec){
	...
	for (int i = 0; i < data->size - nspace; i++){
		validChars.push_back(i);
	}
	
	// cout << "\nValid chars:" << endl;
	// for (int i = 0; i < validChars.size(); i++){
	// 	cout << data->char_list[validChars[i]] << " ";
	// }
	// getchar();
	...
	for (int i = 0; i < validChars.size(); i++){
		...
		for (int j = 0; j < validChars.size(); j++){
			if (j != i){
				...
				temp_cost = multispaceCalc(data, keyb);

				deltaFi = temp_cost - iniCost;

				// cout << "\nDeltafi: " << deltaFi
				// 	 << "\nchar1: " << data->char_list[i]
				// 	 << "\nchar2: " << data->char_list[j]
				// 	 << endl;
				// 	 getchar();

				// cout << "\nAssignments:" << endl; 
				// for (int i = 0; i < data->size; i++){
				// 	cout << " | Char: " << data->char_list[i] << "// Assigned: " << keyb->assignment[i];
				// }
				// getchar();
	...
	if (imp > 0){
		...
		keyb->cost = iniCost + bestDeltaFi;
		// cout << "\nNormal cost calc: " << keyb->cost << endl;
		// getchar();
	}
	...
	// long long testCost;
	// testCost = multispaceCalc(data, keyb);
	// if (keyb->cost == testCost){
	// 	cout << "\nOk (swap): " << keyb->cost << endl;
	// }
	// else{
	// 	cout << "\nWRONG VALUES (swap)" << endl;
	// }
...
void mswapPair (Gendata *data, Keyboard *keyb, Stats *stat, vector<int>& bSpaces, vector<int>& spaceVec, vector<int>& charVec){
	...
	for (int i = 0; i < charVec.size(); i++){
		//if (keyb->grid[charVec[i]] > -1){
			spairs.pos = charVec[i];
			spairs.vert = 0;
			spairs.horiz = 0;

			if (floor((charVec[i] + keyb->kcols) / keyb->kcols) < 5){
				if (keyb->grid[charVec[i] + keyb->kcols] < data->size - nspace){
					spairs.vpair = charVec[i] + keyb->kcols;
					spairs.vert = 1;
				}
			}
			if ((charVec[i] + 1) % keyb->kcols > 0 && (charVec[i] + 1) % keyb->kcols < 6){
				if (keyb->grid[charVec[i] + 1] < data->size - nspace){
					spairs.hpair = charVec[i] + 1;
					spairs.horiz = 1;
				}
			}
			vecpairs.push_back(spairs);
	...
	// cout << "\nPair list: " << endl;
	// for (int i = 0; i < vecpairs.size(); i++){
	// 	cout << "Char: " << data->char_list[keyb->grid[vecpairs[i].pos]] << " - Vert: " << data->char_list[keyb->grid[vecpairs[i].vpair]]
	// 		 << " - Hor: " << data->char_list[keyb->grid[vecpairs[i].hpair]] << endl;
	// }
	// getchar();
		...
		for (int j = i + 1; j < vecpairs.size(); j++){
			if (vecpairs[i].vert > 0 && vecpairs[j].vert > 0){
				if (vecpairs[j].pos != vecpairs[i].vpair){
					...
					temp2 = keyb->assignment[keyb->grid[vecpairs[i].vpair]];
					keyb->assignment[keyb->grid[vecpairs[i].vpair]] = keyb->assignment[keyb->grid[vecpairs[j].vpair]];
					keyb->assignment[keyb->grid[vecpairs[j].vpair]] = temp2;

					temp_cost = multispaceCalc(data, keyb);

					deltaFi = temp_cost - iniCost;

					s_Cost = iniCost + deltaFi;
					...
					temp2 = keyb->assignment[keyb->grid[vecpairs[j].vpair]];
					keyb->assignment[keyb->grid[vecpairs[j].vpair]] = keyb->assignment[keyb->grid[vecpairs[i].vpair]];
					keyb->assignment[keyb->grid[vecpairs[i].vpair]] = temp2;
				}
			}
			...
			if (vecpairs[i].horiz > 0 && vecpairs[j].horiz > 0){
				if (vecpairs[j].pos != vecpairs[i].hpair){
					...
					temp2 = keyb->assignment[keyb->grid[vecpairs[i].hpair]];
					keyb->assignment[keyb->grid[vecpairs[i].hpair]] = keyb->assignment[keyb->grid[vecpairs[j].hpair]];
					keyb->assignment[keyb->grid[vecpairs[j].hpair]] = temp2;
					...
					temp2 = keyb->assignment[keyb->grid[vecpairs[j].hpair]];
					keyb->assignment[keyb->grid[vecpairs[j].hpair]] = keyb->assignment[keyb->grid[vecpairs[i].hpair]];
					keyb->assignment[keyb->grid[vecpairs[i].hpair]] = temp2;
				}
			}
		}//for j
	}//for i
	...
	// if (keyb->cost == testCost){
	// 	cout << "\nOk (swappair): " << keyb->cost << endl;
	// }
	// else{
	// 	cout << "\nWRONG VALUES (swappair)" << endl;
	// }
...
void mtSF_RVND (Gendata *data, Keyboard *keyb, Stats *stat, vector<int>& bSpaces, vector<int>& spaceVec, vector<int>& charVec){
	...
	while (!nbrList.empty()) {
		...
		switch (nbr) {
			case 1:
				mswap (data, keyb, stat, bSpaces, spaceVec, charVec);
				break;

			case 2:
				mswapPair (data, keyb, stat, bSpaces, spaceVec, charVec);
				break;
...
void mtSFpert1 (Gendata *data, Keyboard *keyb, vector<int>& bSpaces, vector<int>& spaceVec, vector<int>& charVec){
	...
	keyb->cost = multispaceCalc(data, keyb);

	// cout << endl << "\nSolucao Perturbada1: " << endl;
	// printSol(data, keyb, auxKeyb);
	// getchar();
	// cout << endl;
	// cout << "\nCusto perturbacao: " << keyb->cost << endl;
...
void mperturbation (Gendata *data, Keyboard *keyb, int *pertC, vector<int>& bSpaces, vector<int>& spaceVec, vector<int>& charVec){
	...
	switch (pert) {
		case 1:
			mtSFpert1 (data, keyb, bSpaces, spaceVec, charVec);
			*pertC = 1;
			break;

		case 2:
			mtSFpert2 (data, keyb, bSpaces, spaceVec, charVec);
			*pertC = 2;
			break;
