...
void swapPair (Gendata *data, Keyboard *keyb, Stats *stat){ //nova versão (com equação de Stutzle e Dorigo)
	...
	for (int i = 0; i < keyb->ksize - 1; i++){
		if (keyb->grid[i] > -1 && keyb->grid[i] != data->size -1){
			spairs.pos = i;
			spairs.vert = 0;
			spairs.horiz = 0;

			if (floor((i + keyb->kcols) / keyb->kcols) < keyb->krows){
				if (keyb->grid[i + keyb->kcols] > -1 && keyb->grid[i + keyb->kcols] != data->size -1){
					spairs.vpair = i + keyb->kcols;
					spairs.vert = 1;
				}
			}
			if ((i + 1) % keyb->kcols > 0){
				if (keyb->grid[i + 1] > -1 && keyb->grid[i + 1] != data->size -1){
					spairs.hpair = i + 1;
					spairs.horiz = 1;
				}
			}
